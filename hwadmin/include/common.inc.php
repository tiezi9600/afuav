...
if(isset($akpth)) {
	define('AK_ROOT', $akpath);
} else {
	define('AK_ROOT', substr(dirname(__FILE__), 0, -7));
}
if(file_exists(AK_ROOT.'configs/config.inc.php') && ($__callmode == 'command' || (!preg_match('/\/install.php/i', $currenturl) && !preg_match('/\/release.php/i', $currenturl)))) {
	require_once AK_ROOT.'configs/config.inc.php';
} else {
	$template_path = 'ak';
	$timedifference = 0;
	$charset = 'utf8';
...
if(isset($ak_root)) define('AK_ROOT', $ak_root);
...
require_once AK_ROOT.'include/render.inc.bin';
...
if(isset($fore_root)) define('FORE_ROOT', $fore_root);
...
if($__callmode == 'web') {
	...
	if(!isset($ak_url)) define('AK_URL', $fore_url.$system_root.'/');
	unset($_p1, $_p2, $_u1);
}
...
if(!preg_match('/\/install.php/i', $_SERVER['SCRIPT_NAME']) && !preg_match('/language.php/i', $_SERVER['SCRIPT_NAME'])) {
...
//if(akgetcookie('auth')) $admin_id = authcode(akgetcookie('auth'), 'DECODE');
$onlineip = '127.0.0.1';
if('web' == $__callmode) {
