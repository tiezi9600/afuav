...
function addwatermark($source, $sourcesize = array()) {
	...
	if(empty($sourcesize)) $sourcesize = getimagesize($source);
...
function getthumbofpicture($picture, $tw, $th = 0) {
	global $setting_attachimagequality, $setting_thumbmethod;
	$path = pathinfo($picture);
	...
	ak_touch($thumb);
	imagejpeg($targetimg, $thumb, $setting_attachimagequality);
	return $thumb;
}
?>