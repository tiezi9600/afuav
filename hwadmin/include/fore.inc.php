...
$db = db();
require(AK_ROOT.'include/smarty/libs/Smarty.class.php');

function fore404() {
	if(!empty($GLOBALS['iflog'])) error_log($get_filename."\n", 3, AK_ROOT."logs/rounter-error".date('Ymd', $GLOBALS['thetime']).".txt");
...
function foredisplay($id, $type = 'item', $template = '') {
	...
	if($type == 'item') {
		$variables = get_item_data($id, $template);
