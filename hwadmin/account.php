	...
	if(!isset($get_job)) {
		...
		while($user = $db->fetch_array($query)) {
			if($user['editor'] != 'admin') {
				...
				$changestatus = empty($user['freeze']) ? "<a href=\"account.php?action=manageaccounts&vc={$vc}&job=freeze&id={$user['id']}\">{$lan['freeze']}</a>" : "<a href=\"account.php?action=manageaccounts&vc={$vc}&job=active&id={$user['id']}\">{$lan['activate']}</a>";
				$reset = "<a href=\"account.php?action=manageaccounts&vc={$vc}&job=reset&id={$user['id']}\">".alert($lan['reset'])."</a>";
				if($user['items'] == 0) {
					$delete = "<a href=\"account.php?action=manageaccounts&job=delete&vc={$vc}&editor={$user['editor']}\">".alert($lan['delete'])."</a>";
				} else {
					$delete = "-";
				}
			} else {
		...
		adminmsg($lan['accoundpassword']."{$post_account}/{$post_password}<br>".$lan['operatesuccess'], 'account.php?action=manageaccounts');
		...
		adminmsg($lan['operatesuccess'], 'account.php?action=manageaccounts');
		...
		adminmsg($lan['passwordreset'], 'account.php?action=manageaccounts');
	}
